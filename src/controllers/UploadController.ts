...
export class UploadController {
  ...
  static async uploadImage(req: Request, res: Response) {
    try {
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        return ApiResponse.error(res, error.message || 'Failed to upload image', 500)
      })
    } catch (error: any) {
  ...
  static async deleteImage(req: Request, res: Response) {
    try {
      ...
      if (result.result === 'ok') {
        return ApiResponse.success(res, { deleted: true }, 'Image deleted successfully')
      } else {
  ...
  static async uploadInvoice(req: Request, res: Response) {
    try {
      ...
      if (!project.assigned_collaborator) {
        return ApiResponse.error(res, 'Project must be assigned to a collaborator before uploading invoice', 400)
      }
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        return ApiResponse.error(res, error.message || 'Failed to upload invoice', 500)
      })
    } catch (error: any) {
      return ApiResponse.error(res, error.message, 500)
    }
  ...
  static async serveInvoice(req: Request, res: Response) {
    try {
      const { projectId } = req.params
      ...
      if (invoicePublicId) {
        try {
          ...
          return new Promise<void>((resolve, reject) => {
            ...
            const request = https.get(parsedUrl.href, (response: any) => {
            ...
            return ApiResponse.error(res, error.message || 'Failed to serve invoice', 500)
          })
        } catch (cloudinaryError: any) {
          console.error('Error accessing Cloudinary file:', cloudinaryError)
          return ApiResponse.error(res, cloudinaryError.message || 'Failed to serve invoice', 500)
        }
      } else {
  ...
  static async uploadMonthlyInvoice(req: Request, res: Response) {
    try {
      if (!req.file) {
        return ApiResponse.error(res, 'No file uploaded', 400)
      }
      ...
      if (projects.length === 0) {
        return ApiResponse.error(res, `No completed projects found for month ${month}`, 400)
      }
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        return ApiResponse.error(res, error.message || 'Failed to upload monthly invoice', 500)
      })
    } catch (error: any) {
      return ApiResponse.error(res, error.message, 500)
    }
